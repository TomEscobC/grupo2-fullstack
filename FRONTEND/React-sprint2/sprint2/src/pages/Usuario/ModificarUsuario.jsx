...
const EditUser = () => {
  ...
  // Recuperar el ID del usuario del localStorage al cargar el componente
  useEffect(() => {
  ...
  const validar = () => {
    let formErrors = {};
    if (!formData.id) formErrors.id = "Requerido";
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(formErrors).length === 0) {
      try {
        ...
        if (!token) {
          alert("Debes iniciar sesión para modificar un usuario.");
          return;
        }

        console.log("Token enviado al backend:", token);

        // Usamos el ID del usuario logeado desde el estado
        const response = await fetch(`http://localhost:3000/api/register/${formData.id}`, {
          method: "PUT",
          headers: {
